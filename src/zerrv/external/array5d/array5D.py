...
class RawShape:
    def __init__(
        ...
        *,
        ...
        self.index_map = {
            axis: idx for axis, idx in zip("tcxyz", (t, c, x, y, z)) if idx is not None
        }
        self.shape = shape
    ...
    def to_shape_tuple(
        self, *, with_t=None, with_c=None, with_x=None, with_y=None, with_z=None
    ):
    ...
    def swapped(self, source: str, destination: str):
        ...
        for source_key, destination_key in zip(source, destination):
            d[source_key], d[destination_key] = (
                self.index_map[destination_key],
                self.index_map[source_key],
            )
...
class Array5D:
    ...
    @classmethod
    def allocate(
        cls, shape: Shape5D, dtype, axiskeys: str = Point5D.LABELS, value: int = None
    ):
    ...
    def rawshape(self):
        return RawShape(
            self.shape, **{axis: idx for idx, axis in enumerate(self.axiskeys)}
        )
    ...
    def normalized(self, iteration_axes: str = "tzc") -> "Array5D":
        ...
        for indices in itertools.product(*axis_ranges):
            ...
            if data_range != 0:
                dest_slice[...] = (
                    source_slice / data_range * np.iinfo(self.dtype).max
                ).astype(self.dtype)
            else:
    ...
    def moveaxis(self, source: str, destination: str):
        ...
        moved_arr = np.moveaxis(
            self._data, source=source_indices, destination=dest_indices
        )
        return self.rebuild(
            moved_arr, axiskeys=self.rawshape.swapped(source, destination).axiskeys
        )
    ...
    def raw(self, axiskeys: str) -> np.ndarray:
        ...
        assert all(
            self.shape[axis] == 1 for axis in Point5D.LABELS if axis not in axiskeys
        )
