...
class LazyArray5D:
    ...
    def cut(self, roi: Slice5D) -> "Array5D":
        ...
        slices = roi.to_slices(self._original_axiskeys)
        logger.debug(f"original_axiskeys: {self._original_axiskeys}")
        logger.debug(f"cutting with slices: {slices}")
        return Array5D(self._data[slices][self._slices], self.axiskeys)
